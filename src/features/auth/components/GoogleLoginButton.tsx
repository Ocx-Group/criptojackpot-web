...
const GoogleLoginButtonInner = () => {
  ...
  const googleLoginMutation = useMutation({
    mutationFn: (request: GoogleLoginRequest) => getAuthService().googleLogin(request),
    onSuccess: data => {
    ...
    onError: (error: Error) => {
      ...
      const message = axiosError.response?.data?.message || t('LOGIN.errors.googleLoginFailed', 'Error al iniciar sesión con Google');
    ...
    >
      ...
      <span className="fw_600" style={{ color: '#3c4043' }}>
        {googleLoginMutation.isPending
          ? t('LOGIN.loading')
          : t('LOGIN.googleLogin', 'Continuar con Google')}
      </span>
    </button>
  );
};
